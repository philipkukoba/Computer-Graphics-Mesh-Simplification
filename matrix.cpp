...
bool Matrix::isInvertible(float epsilon, float det) const {
...
bool Matrix::isInvertible(float epsilon) const {
	return isInvertible(epsilon, Determinant());
}

int Matrix::InverseGivenDet(Matrix& m, float epsilon, float det) const {
	m = *this;
